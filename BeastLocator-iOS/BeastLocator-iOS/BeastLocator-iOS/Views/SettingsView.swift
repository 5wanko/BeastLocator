...
struct SettingsView: View {
                ...
                if store.isDebugMenuVisible {
                    Section(header: Text("debug_section_title")) {
                        ...
                        Button("debug_reset_distance_button") {
                            ...
                            locationManager.waitingForLocation = true
                        }
                    }
                }
            }
    ...
    private func buildMockCurrentLocation(destination: CLLocationCoordinate2D, distanceMeters: Double) -> CLLocationCoordinate2D {
        if distanceMeters <= 0 { return destination }
        
        var estimatedMeters = distanceMeters
        var candidate = offsetFromDestination(destination: destination, distanceMeters: estimatedMeters, bearingDegrees: 180.0)
        
        for _ in 0..<5 {
            let actualMeters = GeoUtils.shared.distanceMeters(from: candidate, to: destination)
            let errorMeters = distanceMeters - actualMeters
            if abs(errorMeters) < 1.0 {
                return candidate
            }
            let safeActual = max(1.0, actualMeters)
            estimatedMeters = max(0.0, estimatedMeters * (distanceMeters / safeActual))
            candidate = offsetFromDestination(destination: destination, distanceMeters: estimatedMeters, bearingDegrees: 180.0)
        }
        return candidate
    }
    
    private func offsetFromDestination(destination: CLLocationCoordinate2D, distanceMeters: Double, bearingDegrees: Double) -> CLLocationCoordinate2D {
        let earthRadius = 6371000.0
        let angularDistance = distanceMeters / earthRadius
        let bearing = bearingDegrees * Double.pi / 180.0
